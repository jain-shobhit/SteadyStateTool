function [OMEGA, SOL, PICARD] = sequential_continuation_reformulated(O,range,varargin)
...
x0 = p.Results.init;
if ~isempty(x0)
    z0 = - O.U.' * SSR.evaluate_fun_over_array(O.S,x0,false);
end
...
for j = 1:n_intervals
    ...
    for k = 1:n_Omega
        ...
        if try_Picard
            try
                [x0, xd0, z0] = Picard_reformulated(O,'init',z0);                
            catch
                disp('Switching to Newton--Raphson iteration')
                try_Picard = false;
                [x0, xd0, z0] = NewtonRaphson_reformulated(O,'init',z0);
            end
        else
            [x0, xd0, z0] = NewtonRaphson_reformulated(O,'init',z0);
        end
